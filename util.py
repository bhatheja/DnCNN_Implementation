...
def ssim_values(dataloader, model, device, epoch):
    ...
    for ite, (clean_image, noisy_image, _) in enumerate(dataloader):
        ...
        if epoch!=0:
            ...
            output = output.permute(0,2,3,1).detach().cpu().numpy()
            output = retrieve_image(output, patch_size=128, img_size_height = h, img_size_width = w)
            similarity_index, _ = ssim(output, clean_image, full=True, data_range = 255, channel_axis=2)
            ssim_val.append(similarity_index)
        else:
            similarity_index, _ = ssim(noisy, clean_image, full=True, data_range = 255, channel_axis=2)
            ssim_val.append(similarity_index)
    return ssim_val
...
def defect_ssim_psnr(directory, len_data = 88, max_value = 255.0, evaluation = True, masked_defect_image_save = True):
